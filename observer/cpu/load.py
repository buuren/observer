...
class LoadCPUStats:
    def __init__(self, observer, metric_key):
        ...
        self.observer.observer_instances[self.metric_key]["load"] = self
    ...
    def calculate_values(self, index):
        ...
        self.observer.raw_values[self.metric_key][index] = cpu_stats

    def generate_counters(self, index):
        read_cpu_stats = self.observer.file_content[index][self.metric_key]['/proc/stat'][:os.cpu_count() + 1]
